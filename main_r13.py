...
ax1.plot(years, births, color='skyblue', marker='o', label='Births', linewidth=3)
...
# Set the x-axis limits so the lines touch the y-axes
ax1.set_xlim([pd.to_datetime('1841-01-01'), pd.to_datetime('1849-12-31')])
...
# Add gridlines only for the major ticks (years)
ax1.grid(True, which='major', linestyle='--', color='gray', alpha=0.7)

# Create a secondary y-axis to plot the number of deaths
ax2 = ax1.twinx()
ax2.set_ylabel('Number of Deaths', color='tab:red')
ax2.plot(years, deaths, color='crimson', marker='o', label='Deaths', linestyle='--', linewidth=2)
